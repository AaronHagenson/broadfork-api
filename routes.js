...
router.get('/testget', function(req, res, next) {
	...
	database.connect(config.testMongoDBURI)
	.then(
		function() {
			return database.getQuote(config.quotesCollection)
		}) 	
	.then(
		function(doc) {
			return {
		...
		function(error) {
			console.log('Failed to find quote: ' + error);
			return {
					"success": false,
					"name": "",
					"quote": "",
					"error": "Failed to find last quote: " + error
				};
		})
	.then(
